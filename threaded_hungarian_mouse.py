    ...
    with open(timing_logname, "w") as fid:
        ...
        if opts["flags"].reweight is True:
            ...
            label_weight = np.asarray(
                [1 for i in range(samplers[0].label_dims + 1)]
            ).astype('float64')
        toc = time.time()
...
def wasserstein(opts, step, y, yhat, frame_mask):
    ...
    # mask = torch.tensor(
    #     y_sum == 0, dtype=torch.float32, requires_grad=False).cuda()
    y = temp_y / (y_sum + eps)

    # yhat_sum = yhat.sum(0).detach()
    temp_yhat = yhat * frame_mask
    yhat_sum = temp_yhat.sum(0)

    # mask = torch.tensor(
    #     yhat_sum == 0, dtype=torch.float32, requires_grad=False).cuda()
...
def _setup_templates(opts, data_files, label_names):
    ...
    if len(data_files) == 3 and data_files[2] is not None:
        sequences_helper.copy_templates(
